...
export default function ManualSchedule() {
          ...
          <div className="space-y-4">
            {schedules.map((post, index) => {
              const imageFile = imageFiles.find((file) =>{
                 file.name.toLowerCase().includes(`post ${index + 1}`)
                console.log(file);
              });
              
              const textFile = textFiles.find((file) => file.length > 0);
              ...
              return (
                ...
                >
                  <h3 className="font-semibold text-indigo-600 mb-2">📦 Post {index + 1}</h3>

                  <ul className="text-sm mb-2 text-gray-700 space-y-1">
  <li>
    📸 Image: {imageFile ? (
      <span className="text-green-600">
        ✅ Post {index + 1} - <span className="font-medium">{imageFile.name}</span>
      </span>
    ) : (
  ...
  <li>
    📝 Text: {textFile ? (
      <span className="text-green-600">
        ✅ Post {index + 1} - <span className="font-medium">{textFile.name}</span>
      </span>
    ) : (
      <span className="text-red-500">❌ Not found</span>
    )}
